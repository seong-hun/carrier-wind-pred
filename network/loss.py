...
class LossEG(nn.Module):
    ...
    def loss_adv(self, r_x_hat, D_res_list, D_hat_res_list):
        ...
        for res, res_hat in zip(D_res_list, D_hat_res_list):
            ...
            res_hat = res_hat.to(self.device)
            loss_fm += F.l1_loss(res, res_hat)
        return -r_x_hat.mean() + loss_fm * args.LOSS_FM_WEIGHT

    def loss_mch(self, e_hat, W_i):
        return F.l1_loss(
            W_i.reshape(-1), e_hat.reshape(-1)) * args.LOSS_MCH_WEIGHT

    def forward(self, x, x_hat, r_x_hat,
                D_res_list, D_hat_res_list, e_hat, W_i):
        x = x.to(self.device)
        ...
        adv = self.loss_adv(r_x_hat, D_res_list, D_hat_res_list)
