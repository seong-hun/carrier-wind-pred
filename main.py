...
xgrid = np.linspace(*xlim, GRIDSIZE)
ygrid = np.linspace(*ylim, GRIDSIZE)
xmap, ymap = np.meshgrid(xgrid, ygrid)

terrain = Terrain()
basewind = WindShear(6, 6)
...
for h, w in zip(hlist, wmap):
    ...
    w[np.logical_and(w > -0.6, w < 0.6)] = np.nan
    w = h + w
